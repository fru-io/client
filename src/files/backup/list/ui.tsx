...
import { BackupFilesRequest, BackupFilesResponse, FileBackupMetadata, ListFileBackupsRequest, ListFileBackupsResponse } from '@fru-io/fru-apis/live/sites/v1alpha1/file_pb';
...
export const DefaultComponent: FC<Props> = (props) => {
	const [resp, setResp] = useState<ListFileBackupsResponse>()
	const [err, setError] = useState<grpc.ServiceError>()
	useEffect(() => {
		// Create an scoped async function in the hook
		async function init() {
			client.listFileBackups(props.req, props.meta, (error: grpc.ServiceError | null, value?: ListFileBackupsResponse) => {
				if (error) {
					setError(error)
				}
				setResp(value)
	  ...
	  if (resp) {
		  return (
			  <Box>
				  {
					  resp.getBackupsList().map( (meta:FileBackupMetadata, index:number) => (
